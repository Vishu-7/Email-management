...
class EmailClientApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Email Audit Client")
    ...
    def build_login_tab(self):
        ...
        self.email_entry.grid(row=1, column=1, padx=15, pady=15)
    ...
    def build_mail_audit_tab(self):
        ...
        ttk.Label(filter_frame, text="Select Time Period:").pack(side="left", padx=5)
        self.time_period_combo = ttk.Combobox(
            filter_frame, 
            values=["Last 1 Hour", "Last 1 Day", "Last 1 Month", 
                   "Last 3 Months", "Last 6 Months", "Last 1 Year"]
        )
        self.time_period_combo.pack(side="left", padx=5)
    ...
    def fetch_emails(self):
        ...
        selected_period = self.time_period_combo.get()
        ...
        try:
            ...
            date_limit = self.calculate_date_limit(selected_period)
            query_date = date_limit.strftime("%d-%b-%Y")
            
            # Search emails
            status, messages = self.mail.search(None, f'SINCE "{query_date}"')
            
            if status != "OK":
                messagebox.showerror("Error", "No emails found for the selected time period.")
                return
            ...
            # Process emails
            self.process_emails(messages[0].split())
    ...
    def calculate_date_limit(self, selected_period):
        periods = {
        ...
        return datetime.now() - periods[selected_period]

    def process_emails(self, message_nums):
        email_threads = {}
        
        # First pass: collect threads
        ...
        # Second pass: display emails
        for idx, (message_id, thread_info) in enumerate(email_threads.items(), 1):
            msg = thread_info['msg']
            ...
            # Get email body for categorization
